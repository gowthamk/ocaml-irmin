...
let alice_uri = "git+ssh://opam@172.18.0.2/tmp/repos/rbset.git"
let bob_uri = "git+ssh://opam@172.18.0.3/tmp/repos/rbset.git"
let uris = [alice_uri; bob_uri]
...
let cheryl_f : unit Vpst.t = 
  loop_until_y "Cheryl: Ready?" >>= fun () ->
  main_loop >>= fun v -> 
  let elts = RBSet.elements v in
  let _ = Printf.printf "Chery: \n" in
  begin
    List.iter (fun x -> 
        printf "%ld; " x) elts;
    Vpst.return ()
