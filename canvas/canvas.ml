module Make = struct
  ...
  type node = {tl_t: t; tr_t: t; bl_t: t; br_t: t}

  and t = N of pixel | B of node
  ...
  let rec set_px canvas loc px =
    if canvas.max_x <= loc.x && canvas.max_y <= loc.y then N px
    else
      ...
      | true, true ->
          (* top-left quadrant *)
          let tl_t = match canvas.t with N px -> N px | B {tl_t} -> tl_t in
          let tl_c = {max_x= mid_x; max_y= mid_y; t= tl_t} in
          let tl_t' = set_px tl_c loc px in
          let t' =
            match canvas.t with
            | N px -> B {tl_t= tl_t'; tr_t= N px; bl_t= N px; br_t= N px}
            | B y -> B {y with tl_t= tl_t'}
          in
          t'
      | false, true ->
          (* top-right quadrant *)
          let tr_t = match canvas.t with N px -> N px | B {tr_t} -> tr_t in
          let tr_c = {max_x= canvas.max_x - mid_x; max_y= mid_y; t= tr_t} in
          let loc' = {loc with x= loc.x - mid_x} in
          let tr_t' = set_px tr_c loc' px in
          let t' =
            match canvas.t with
            | N px -> B {tl_t= N px; tr_t= tr_t'; bl_t= N px; br_t= N px}
            | B y -> B {y with tr_t= tr_t'}
          in
          t'
      | true, false ->
          (* bottom-left quadrant *)
          let bl_t = match canvas.t with N px -> N px | B {bl_t} -> bl_t in
          let bl_c = {max_x= mid_x; max_y= canvas.max_y - mid_y; t= bl_t} in
          let loc' = {loc with y= loc.y - mid_y} in
          let bl_t' = set_px bl_c loc' px in
          let t' =
            match canvas.t with
            | N px -> B {tl_t= N px; tr_t= N px; bl_t= bl_t'; br_t= N px}
            | B y -> B {y with bl_t= bl_t'}
          in
          t'
      | false, false ->
          (* bottom-right quadrant *)
          let br_t = match canvas.t with N px -> N px | B {br_t} -> br_t in
          let br_c =
            {max_x= canvas.max_x - mid_x; max_y= canvas.max_y - mid_y; t= br_t}
          in
          let loc' = {x= loc.x - mid_x; y= loc.y - mid_y} in
          let br_t' = set_px br_c loc' px in
          let t' =
            match canvas.t with
            | N px -> B {tl_t= N px; tr_t= N px; bl_t= N px; br_t= br_t'}
            | B y -> B {y with br_t= br_t'}
          in
          t'
  ...
  let make_b (tl, tr, bl, br) = B {tl_t= tl; tr_t= tr; bl_t= bl; br_t= br}
  ...
  let rec merge old v1 v2 =
    if v1 = v2 then v1
      ...
      | _, N px1, N px2 ->
          ...
          let px' = color_mix px1 px2 in
          N px'

  let rec merge_canvas {t=old;max_x;max_y} {t=v1;_} {t=v2;_} =
    {max_x; max_y; t = merge old v1 v2}

  let pp = Format.fprintf
  let rec print ppf min_x min_y max_x max_y t =
    if min_x > max_x || min_y > max_y then ()
      ...
      | N px when not (px = default_pixel) ->
          if min_x = max_x && min_y = max_y then
            ...
            pp ppf "<%d,%d> to <%d,%d>: (%ld,%ld,%ld)\n" min_x min_y
              max_x max_y px.r px.g px.b
      | N px -> ()
      | B {tl_t; tr_t; bl_t; br_t} ->
          ...
          in
          print ppf min_x min_y mid_x mid_y tl_t ;
          print ppf (mid_x + 1) min_y max_x mid_y tr_t ;
          print ppf min_x (mid_y + 1) mid_x max_y bl_t ;
          print ppf (mid_x + 1) (mid_y + 1) max_x max_y br_t
  ...
  let print pp ppf {max_x; max_y; t} = print ppf 0 0 max_x max_y t

  let print ppf c =
    for x = 1 to c.max_x do
      for y = 1 to c.max_y do
        let px = get_px c {x; y} in
        if not (px = default_pixel) then
          pp ppf "<%d,%d>: (%ld,%ld,%ld)\n" x y px.r px.g px.b
        else ()
      done
    done

  let print_stdout = print (Format.formatter_of_out_channel stdout)
end
