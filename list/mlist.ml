...
module Make (Atom : ATOM) = struct
  ...
  let populate_table t l = List.iter (fun a -> Hashtbl.add t a true) l

  let populate_removes xs =
    let remove a = Hashtbl.add removes a true in
    List.iter
      (fun x -> if contains yt x && contains zt x then () else remove x)
      xs
  ...
  let rec two_way_merge ys zs =
    match (ys, zs) with
    | y :: ys', z :: zs' ->
        if y <= z then y :: two_way_merge ys' zs else z :: two_way_merge ys zs'
  ...
  let merge xs ys zs =
    ...
    let zs' =
      List.filter (fun z -> (not @@ removed z) && (not @@ contains yt z)) zs
    in
