...
module Make (Atom : ATOM) = (* : Mset.S *) struct
  type atom = Atom.t
  ...
  *)
  ...
  let rec partition p = function
    ...
    | Node {l; v; r; _} ->
        ...
        if pv then (join lt v rt, concat lf rf)
  ...
  let rec find_first f = function
    | Empty -> raise Not_found
    | Node {l; v; r; _} -> if f v then find_first_aux v f l else find_first f r
  ...
  let rec find_last f = function
    | Empty -> raise Not_found
    | Node {l; v; r; _} -> if f v then find_last_aux v f r else find_last f l
  ...
  let of_sorted_list l =
    let rec sub n l =
      ...
      | 2, x0 :: x1 :: l ->
          ( Node
              {l= Node {l= Empty; v= x0; r= Empty; h= 1}; v= x1; r= Empty; h= 2}
          , l )
      | 3, x0 :: x1 :: x2 :: l ->
          ( Node
  ...
  (* Patching *)
  type edit = Add of atom | Remove of atom
    ...
    let rec diff_avlt s1 s2 =
      ...
      | Node {l= l1; v= v1; r= r1; h= h1}, Node {l= l2; v= v2; r= r2; h= h2} ->
          if h1 >= h2 then
            ...
            if p then List.append l r else List.append l (Remove v1 :: r)
          else
            let l1, p, r1 = split v2 s1 in
            let l = diff_avlt l1 l2 in
            let r = diff_avlt r1 r2 in
            if p then List.append l r else List.append l (Add v2 :: r)
    in
    diff_avlt xt yt
    ...
    let rec transform_aux xs ys =
      ...
      | hx :: rxs, hy :: rys -> (
          ...
          | Add x, Remove y | Remove x, Add y ->
              ...
              let on_conflict () = assert false in
              handle x y on_conflict )
    in
    transform_aux p q
