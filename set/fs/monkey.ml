...
module MkConfig (Vars: sig val root: string end) : Iset_imp.Config = struct
  let root = Vars.root
 ...
 * --------------------------------------------------------
 *)
module CInit = MkConfig(struct let root = "/tmp/repos/set_imp2.git" end)
module HInit = Iset_imp.MakeVersioned(CInit)(Atom)
module H = Set_imp.Make(Atom)
module Vpst = HInit.Vpst

let bob_uri = "git+ssh://opam@172.18.0.3/tmp/repos/set_imp.git"
...
let loop_iter i (pre: H.t Vpst.t) : H.t Vpst.t = 
  ...
  let t2 = Sys.time () in
  Vpst.sync_next_version ~v:c' uris >>= fun v ->
  let t3 = Sys.time () in
  let b_size = (H.size c') in
  begin 
