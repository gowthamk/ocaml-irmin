...
module MakeVersioned (Config : Config) (Atom : Set_imp.ATOM) = struct
  module OM = Set_imp.Make (Atom)
  module K = Irmin.Hash.SHA1

  let from_just = function Some x -> x | None -> failwith "Expected Some. Got None."
  ...
  module M = struct
    module AO_value = struct
      ...
      let t =
        let open Irmin.Type in
        variant "t" (fun empty node -> function Empty -> empty | Node n -> node n)
        |~ case0 "Empty" Empty
    ...
    let rec of_adt (a : OM.t) : t Lwt.t =
      ...
      let aostore_add value = aostore >>= fun ao_store -> AO_store.add ao_store value in
      aostore_add
      =<<
      match a with
      ...
      | OM.Node {l; v; r; h} ->
        of_adt l
        >>= (fun l' ->
              of_adt r >>= fun r' -> Lwt.return {l = l'; v; r = r'; h = Int64.of_int h}
              )
    ...
    let rec to_adt (k : t) : OM.t Lwt.t =
      ...
      | Node {l; v; r; h} ->
        ...
        >>= (fun l' ->
              to_adt r
              >>= fun r' -> Lwt.return {OM.l = l'; OM.v; OM.r = r'; OM.h = Int64.to_int h}
              )
    ...
    let rec merge ~(old : t Irmin.Merge.promise) v1_k v2_k =
      ...
      of_adt v >>= fun merged_k -> Irmin.Merge.ok merged_k
    ;;
  ...
  module Vpst : sig
   ...
   ;;
    ...
    let with_init_version_do (v : OM.t) (m : 'a t) =
      Lwt_main.run
        ...
        BC_store.update m_br path k ~info:cinfo
        >>= fun () ->
   ...
   ;;
    ...
    let get_latest_version () : OM.t t =
     fun (st : st) ->
      ...
      >>= fun k -> M.to_adt @@ from_just k >>= fun td -> Lwt.return (td, st)
   ;;

    let sync_remote_version remote_uri ?v : OM.t t =
     fun (st : st) ->
      ...
      (match v with
      | None -> Lwt.return ()
      | Some v -> M.of_adt v >>= fun k -> BC_store.update st.local path k cinfo)
      >>= fun () ->
      (* 2.. Pull from remote to master *)
      let cinfo = info (Printf.sprintf "Merging remote: %s" remote_uri) in
      BC_store.Sync.pull st.master (Irmin.remote_uri remote_uri) (`Merge cinfo)
      >>= fun _ ->
      ...
      BC_store.merge st.local ~into:st.master ~info:cinfo
      >>= fun _ -> get_latest_version () st
   ;;

    let sync_next_version ?v : OM.t t =
     fun (st : st) ->
      (* How do you commit the next version? Simply update path? *)
      (* 1. Commit to the local branch *)
      let cinfo = info "committing local state" in
   ...
   ;;
    ...
    let liftLwt (m : 'a Lwt.t) : 'a t = fun st -> m >>= fun a -> Lwt.return (a, st)
  end
end
