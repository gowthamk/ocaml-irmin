...
module Make (Atom : ATOM) = struct
  ...
  let add x q =
    ...
    | Cons last ->
        last.next <- cell ;
        {length= q.length + 1; first= q.first; last= cell}

  (* push represents the push operation which basically pushes an element to the queue *)
  let push = add

  let nthq q n =
    if n < 0 then invalid_arg "Queue.nthq"
    else
      let rec nthq_aux q n =
        match q.first with
        | Nil -> raise Empty
        | Cons {content; next= Nil} ->
            if n = 0 then content else invalid_arg "Queue range"
        | Cons {content; next} ->
            if n = 0 then content
            else
              nthq_aux {length= q.length - 1; first= next; last= next} (n - 1)
      in
      nthq_aux q n
  ...
  let peek q =
    match q.first with Nil -> raise Empty | Cons {content; next} -> content
  ...
  let take q =
    match q.first with
    | Nil -> raise Empty
    | Cons {content; next= Nil} -> clear q ; content
    | Cons {content; next} ->
        q.length <- q.length - 1 ;
        q.first <- next ;
        content
  ...
  let rec equal q1 q2 =
    if get q1 0 = get q2 0 then equal (q_after_take q1) (q_after_take q2)
    else false
  ...
  let to_list q = List.rev (fold (fun acc e -> e :: acc) [] q)
  ...
  module type S = sig
    (* Patching *)
    type edit = Add of atom | Take of atom
  ...
  let get_edit_atom e = match e with Add nx -> nx | Take nx -> nx

  let rec delete x l =
    match l with [] -> [] | y :: l' -> if x = y then l' else y :: delete x l'

  let rec take_upto_index i q =
    if i = 0 then q_after_take q else take_upto_index (i - 1) (q_after_take q)

  let rec compare_till_index i q1 q2 =
    if i = 0 then if get q1 0 = get q2 0 then true else false
    else compare_till_index (i - 1) q1 q2

  let rec compare_q q1 q2 =
    match (q1, q2) with
    ...
    | xs, ys ->
        if get xs 0 = get ys 0 then
          compare_q (q_after_take xs) (q_after_take ys)
        else false
  ...
  let rec op_diff xt yt =
    let rec diff_avlt s1 s2 =
      ...
      | {length= 0; first= Nil; last= Nil}, {length= 0; first= Nil; last= Nil}
        ->
          []
      | xs, {length= 0; first= Nil; last= Nil} ->
          Take (get xs 0)
          :: diff_avlt (q_after_take xs) {length= 0; first= Nil; last= Nil}
      | {length= 0; first= Nil; last= Nil}, ys ->
          Add (get ys 0)
          :: diff_avlt {length= 0; first= Nil; last= Nil} (q_after_take ys)
      | xs, ys ->
          let c = compare (length xs) (length ys) in
          if c > 0 then
            if compare_till_index (length ys - 1) xs ys then
              List.append
                ( Take (get xs 0)
                :: diff_avlt (q_after_take xs)
            ...
            if compare_q xs ys then []
              ...
              List.append
                ...
                :: diff_avlt
                     {length= 0; first= Nil; last= Nil}
                     (q_after_take ys) )
          else if compare_till_index (length xs - 1) xs ys then
            diff_avlt
              {length= 0; first= Nil; last= Nil}
              (take_upto_index (length xs - 1) ys)
          else
            List.append
              ( Take (get xs 0)
              :: diff_avlt (q_after_take xs) {length= 0; first= Nil; last= Nil}
  ...
  let offset = function Add _ -> 1 | Take _ -> -1
  ...
  let rec check_common_el_list x y =
    match (x, y) with
    | [], [] -> true
    | xs, [] -> false
    | [], ys -> false
    | x' :: xs', y' :: ys' ->
        if x' = y' || List.mem x' (y' :: ys') then true
        else check_common_el_list xs' (y' :: ys')

  let rec sublist_upto_index i l =
    match l with
    | [] -> raise (Failure "Not Found")
    | h :: t -> if i = 0 then [] else h :: sublist_upto_index (i - 1) t

  let prev_el_list e l =
    match l with
    | [] -> []
    | x :: xs ->
        let i = find_el e l in
        if i = 0 then [] else x :: sublist_upto_index (i - 1) xs

  let diff l1 l2 = List.filter (fun x -> not (List.mem x l2)) l1

  let rec maintain_order_list l1 l2 =
    match (l1, l2) with
    | [], [] -> []
    | xs, [] -> xs
    | [], ys -> ys
    | x :: xs, y :: ys ->
        if List.mem x (y :: ys) then
          List.append
            (prev_el_list x (y :: ys))
            (List.append [x]
               (maintain_order_list xs
                  (diff (y :: ys) (List.append (prev_el_list x (y :: ys)) [x]))))
  ...
  let rec diff_edit x y =
    ...
    | x', y' when x' = y' -> []
    | Add nx :: xs, Take ny :: ys ->
        remove_from_right (diff_edit (Add nx :: xs) (Take ny :: ys))
    | Take nx :: xs, Add ny :: ys ->
        remove_from_right (diff_edit (Take nx :: ys) (Add ny :: xs))

  let rec diff_append p q =
    match (p, q) with
    | [], [] -> ([], [])
    | xs, [] -> (xs, [])
    | [], ys -> ([], ys)
    | Add nx :: xs, Add ny :: ys ->
        if check_common_el_list (Add nx :: xs) (Add ny :: ys) then
          ( remove_from_right
              (List.append
                 (take_all (Add ny :: ys))
                 (remove_from_left
          ...
          , remove_from_right
              (List.append
                 ...
                 (diff_edit (Add nx :: xs) (Add ny :: ys))) )
    | Add nx :: xs, Take ny :: ys ->
        if check_common_el_list (Add nx :: xs) (delete (Add ny) (Add ny :: ys))
        then
          ( remove_from_right
              (List.append (take_all ys)
                 (remove_from_left
                    (maintain_order_list (add_all (Add nx :: xs)) (add_all ys))))
          , List.append
          ...
          ( remove_from_right (add_all (Add nx :: xs))
          , remove_from_right
              (List.append
                 (take_all (Add ny :: Add nx :: xs))
                 (List.append (add_all ys)
                    (add_all (delete (Add ny) (Add nx :: xs))))) )
        else
          ( delete (Add ny)
              (remove_from_right
                 (List.append (take_all ys)
          ...
          , delete (Add ny)
              (remove_from_right
                 (List.append
                    ...
                    (diff_edit (Add ny :: Add nx :: xs) ys))) )
    | Take nx :: xs, Add ny :: ys ->
        if check_common_el_list xs (delete (Add nx) (Add ny :: ys)) then
          ( remove_from_right
              (List.append
                 (take_all (Add nx :: Add ny :: ys))
                 (remove_from_left
                    (maintain_order_list (add_all xs) (add_all (Add ny :: ys)))))
          ...
          ( remove_from_right
              (List.append
                 (take_all (Add nx :: Add ny :: ys))
                 (List.append
                    (add_all (delete (Add nx) xs))
                    (add_all (Add nx :: Add ny :: ys))))
          , remove_from_right (add_all (Add ny :: ys)) )
        else
          ( delete (Add nx)
              (remove_from_right
                 (List.append
          ...
          , delete (Add nx)
              (remove_from_left
                 (List.append (take_all xs) (diff_edit xs (Add ny :: ys)))) )
    | Take nx :: xs, Take ny :: ys ->
        if
          check_common_el_list xs (delete (Add nx) (Add ny :: ys))
          && (not (List.mem (Add nx) ys))
          && not (List.mem (Add nx) xs)
        then
          ( remove_from_right
              (List.append (take_all ys)
                 (remove_from_left
                    (maintain_order_list (add_all xs) (add_all ys))))
          , List.append (take_all xs)
  ...
  let op_transform p q =
    let rec go xs ys =
      ...
      | x :: xs, y :: ys -> (
        match (x, y) with
        | Add nx, Add ny when nx = ny -> diff_append (x :: xs) (y :: ys)
        | Add nx, Add ny -> diff_append (x :: xs) (y :: ys)
        | Add nx, Take ny ->
            if xs = [] && ys = [] then ([Add nx], [Take ny])
            else diff_append (x :: xs) (y :: ys)
        | Take nx, Add ny ->
            if xs = [] && ys = [] then ([Take nx], [Add ny])
            else diff_append (x :: xs) (y :: ys)
        | Take nx, Take ny ->
            if ys = [] then (fst (diff_append xs []), [])
            else diff_append (x :: xs) (y :: ys) )
    in
    go p q
